...
class Auth extends MY_Controller
{
    ...
    public function _call_back_active_field($val = '', $primary_key = null)
    {
        return form_dropdown('active', array('1' => get_languageword('active'), '0' => get_languageword('inactive')), $val, 'class="chosen-select"');
    }

    public function callback_field_phone_code($val = '', $primary_key = null)
    {
    ...
    public function _call_back_set_action($primary_key, $row)
    {
        $action = "";
    ...
    public function callback_column_user_status($primary_key, $row)
    {
        $txt = "";
        $status = $row->active;
    ...
    public function encrypt_password_and_update_callback($post_array, $primary_key)
    {
    ...
    public function change_approvement_status($user_id, $status)
    {
        if (!$this->ion_auth->logged_in()) {
    ...
    public function groupstatustoggle()
    {
        if ($this->input->post()) {
            $id = $this->input->post('id');
            $ids = explode(',', $id);
    ...
    public function daftar()
    {
        ...
        if (isset($_POST['create'])) {
            ...
            $mail->Password = '$t[OKqUiH.NW'; // password email
            $mail->SMTPSecure = 'ssl';
            $mail->Port = 465;
