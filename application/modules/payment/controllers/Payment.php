...
?>
				<form action="<?php echo site_url('payment/process_stripe/' . $package_id); ?>" method="POST" id="frm_stripe">
					<script src="https://checkout.stripe.com/v2/checkout.js" class="stripe-button" id="stripe_script" data-key="<?php echo $key; ?>" data-amount="<?php echo $final_cost; ?>" data-name="<?php echo $name; ?>" data-description="<?php echo $name; ?>" data-image="<?php if (isset($this->config->item('site_settings')->logo) && $this->config->item('site_settings')->logo != '') echo substr(URL_PUBLIC_UPLOADS_SETTINGS . '' . $this->config->item('site_settings')->logo, 5);
																																																																				else echo substr(URL_FRONT_IMAGES . 'Logo.png', 5); ?>" data-package_id="<?php echo $package_id; ?>" data-currency="<?php echo get_system_settings('Currency_Code'); ?>" data-email="<?php echo $user_info[0]->email; ?>" data-locale="auto">
					</script>
				</form>
<?php
