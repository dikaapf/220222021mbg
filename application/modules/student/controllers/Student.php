...
class Student extends MY_Controller
{
		...
		if ($this->input->post()) {
			if ($this->input->post('student_courses')) {
				...
				$this->prepare_flashmessage(
					get_languageword('please_select_atleast_one_preferred_course'),
					1
				);
			}
	...
	function manage_subjects()
	{
		...
		); //Getting student selected subject ids
	...
	function manage_locations()
	{
		...
		if ($this->input->post()) {
			...
			if ($this->input->post('student_locations')) {
				...
				$this->prepare_flashmessage(
					get_languageword('please_select_atleast_one_preferred_location'),
					1
				);
			}
	...
	function manage_teaching_types()
	{
		...
		if ($this->input->post()) {
			...
			if ($this->input->post('student_selected_types')) {
				...
				$this->prepare_flashmessage(
					get_languageword('please_select_atleast_one_preferred_teaching_type'),
					1
				);
			}
				...
				foreach ($_FILES['certificate']['name'] as $i => $value) {
					if ($_FILES['certificate']['name'][$i] != '') {
						...
						if (move_uploaded_file($tmpFilePath, $filePath)) {
							...
							$existed = $this->base_model->fetch_records_from(
								'users_certificates',
								array(
									'admin_certificate_id' => $i,
