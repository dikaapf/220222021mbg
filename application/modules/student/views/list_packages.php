...
<div class="dashboard-panel">
	...
	if (count($package_data) > 0) {
		// Disable Popup untuk masuk ke topup
	...
	?>
		...
		<div class="custom_accordion1">
			...
			<p class="top20"><b><?php echo get_languageword('Daftar_Paket_:'); ?></b></p>
		...
		<script>
			function assign_price(package_id, final_cost, name) {
				$.post("<?php echo site_url('payment/get_additional_content'); ?>", {
			...
			function get_form(gateway, final_cost) {
				if (gateway == <?php echo RAZORPAY_PAYMENT_GATEWAY; ?>) {
					...
					$.post("<?php echo site_url('payment/get_razorpay_form'); ?>", {
							package_id: package_id,
							final_cost: final_cost
						})
						.done(function(data) {
