 ...
 */
class grocery2_CRUD_Field_Types
{
	/**
 ...
 * @link		http://www.grocerycrud.com/documentation
 */
class grocery2_CRUD_Model_Driver extends grocery2_CRUD_Field_Types
{
	/**
	 * @var Grocery2_crud_model
	 */
	...
	{
		$ci = &get_instance();
		$ci->load->model('Grocery2_crud_model');

		$this->basic_model = new Grocery2_crud_model();
	}
	...
	protected function form_validation()
	{
		...
		{
			$this->form_validation = new grocery2_CRUD_Form_validation();
			$ci = &get_instance();
	...
	protected function upload_file($state_info)
	{
		if(isset($this->upload_fields[$state_info->field_name]) )
		{
			if($this->callback_upload === null)
			{
				...
				$upload_handler = new UploadHandler2($options);
 ...
 * @version    	1.5.4
 */
class grocery2_CRUD_Layout extends grocery2_CRUD_Model_Driver
{
	...
	protected function showList($ajax = false, $state_info = null)
	{
		...
		if(!$ajax)
		{
			...
			$data->list_view = $this->_theme_view('list2.php',$data,true);
			$this->_theme_view('list_template.php',$data);
		}
		else
		{
			$this->set_echo_and_die();
			$this->_theme_view('list2.php',$data);
		}
	}
 ...
 * @version    	1.5.4
 */
class grocery2_CRUD_States extends grocery2_CRUD_Layout
{
 ...
 * @link		http://www.grocerycrud.com/documentation
 */
class Grocery2_CRUD extends grocery2_CRUD_States
{
	/**
	...
	public function render()
	{
		...
		switch ($this->state_code) {
			...
			*/
			...
			case grocery2_CRUD_States::STATE_READ:
				if($this->unset_read)
				{
					throw new Exception('You don\'t have permissions for this operation', 14);
					die();
            ...
            case grocery2_CRUD_States::STATE_DELETE_MULTIPLE:
...
if(defined('CI_VERSION'))
{
	...
	class grocery2_CRUD_Form_validation extends CI_Form_validation{
